    ...
    for(unsigned int ge = 0; ge < nGE; ge++)
    {
        ...
        for (unsigned int gt = 0; gt < nGT; gt++)
        {
            ...
            if(gt == 0)
            {
                ...
                pGT1GE1 = (ALPHANULL + arrTE[3]) / (ALPHANULL + ALPHANULL + arrT[1]);
                pGT0GE1 = (ALPHANULL + arrDE[1] + arrDE[3]) / (ALPHANULL + ALPHANULL + nTumors - arrT[1]);
            }
            else
            {
                //pGT1GE1 = (ALPHAIJK11 + T1ge1) / (ALPHAIJK11 + ALPHAIJK10 + T1);
                //pGT0GE1 = (ALPHAIJK01 + D0ge1 + D1ge1) / (ALPHAIJK01 + ALPHAIJK00 + nTumors - T1);    
                pGT1GE1 = (ALPHAIJK11 + arrTE[3]) / (ALPHAIJK11 + ALPHAIJK10 + arrT[1]);
                pGT0GE1 = (ALPHAIJK01 + arrDE[1] + arrDE[3]) / (ALPHAIJK01 + ALPHAIJK00 + nTumors - arrT[1]);          
            }
...
float calcA0Fscore(float *gt,  float *gtge)
{

    // Calculation of Equation 7    
    //float glnNi0 = lgamma( ALPHANULL + ALPHANULL) - lgamma(gt0 + ALPHANULL + ALPHANULL);
    float glnNi0 = lgamma( ALPHANULL + ALPHANULL) - lgamma(gt[0] + ALPHANULL + ALPHANULL);
    //float glnNi1 = lgamma(ALPHAIJK10 + ALPHANULL) - lgamma(gt1 + ALPHANULL + ALPHANULL);
    float glnNi1 = lgamma(ALPHAIJK10 + ALPHANULL) - lgamma(gt[1] + ALPHANULL + ALPHANULL);

    float fscore = glnNi0 + glnNi1;   
    //fscore += lgamma(gt0ge0 + ALPHANULL) - lgamma(ALPHANULL);
    fscore += lgamma(gtge[0] + ALPHANULL) - lgamma(ALPHANULL);
    //fscore += lgamma(gt0ge1 + ALPHANULL) - lgamma(ALPHANULL);
    fscore += lgamma(gtge[1] + ALPHANULL) - lgamma(ALPHANULL);
    //fscore += lgamma(gt1ge0 + ALPHANULL) - lgamma(ALPHANULL);
    fscore += lgamma(gtge[2] + ALPHANULL) - lgamma(ALPHANULL);
    //fscore += lgamma(gt1ge1 + ALPHANULL) - lgamma(ALPHANULL);
    fscore += lgamma(gtge[3] + ALPHANULL) - lgamma(ALPHANULL);
